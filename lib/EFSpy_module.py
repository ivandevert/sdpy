...
class EFS():
    ...
    def from_obspy(st, evhead={}, invs={}):
        ...
        for ii, tr in enumerate(st):
            ...
            # IV addition 2023-08-31:
            if 'invs' in locals():
                try:
                    tnet, tsta, tloc, tcha = tr.id.split('.')
                    invtmp = invs.select(network=tnet, station=tsta, 
                        location=tloc, channel=tcha)
                    wf['units'] = inv[0][0][0].response.get_paz().input_units
                except:
                    wf['units'] = 'unknown'
